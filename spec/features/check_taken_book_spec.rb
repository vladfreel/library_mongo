...
describe 'add like to book' do
  ...
  let(:user2) { FactoryBot.create(:user, email: 'gggggg@gmail.com') }
  it 'should add book', js: true do
    ...
    fill_in 'pass', with: user2.password
    click_button 'Sign in'
    visit book_path(book)
    expect(page).to have_no_content('Взять')
    expect(page).to have_no_content('Вернуть')
  end
end